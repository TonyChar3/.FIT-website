...
const ProductPage = () => {
    ...
    const AddToWishList = async() =>{
        try{
            const addWish = await addToWishList(id, token)

            if(addWish){

                dispatch(showModal(addWish))
    ...
    const RemoveFromWishList = async() => {
        try{
            const removal = await removeFromWishList(token)
    ...
    const handleAddToCart = async(item) => {
        try{
            ...
            const cartItem = await addItemToCart(hashID, item._id, token)
    ...
    const handleFastCheckout = async(item) => {
        try{
            let itemsArray = [];
